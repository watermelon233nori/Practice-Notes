...
typedef struct _arrayBinaryTree {
    ...
    vector<int> preOrder() {
        return dfsVector_(ORDER_::pre);
    }

    vector<int> inOrder() {
        return dfsVector_(ORDER_::in);
    }
...
private:
    ...
    void dfs(int i, char order, vector<int>& vec) {
        ...
        if (order == this->ORDER_::pre) vec.emplace_back(this->val(i));
        dfs(left(i), order, vec);
        if (order == this->ORDER_::in) vec.emplace_back(this->val(i));
        dfs(right(i), order, vec);
        if (order == this->ORDER_::post) vec.emplace_back(this->val(i));
    }
    inline vector<int> dfsVector_(char order) {
        vector<int> vec;
        vec.reserve(this->tree_.size());
        dfs(0, order, vec);
        vec.shrink_to_fit();
        return vec;
    }
